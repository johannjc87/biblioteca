  ...
  <div class="col-md-6">
    <div class="card">
      ...
      <div class="card-body">
        ...
        <form action="searchLibro.php" method="post">
  Primer nombre:
...
if(!isset($_REQUEST['first'])) {
   echo "<h6> Busque los libros en nuestra Biblioteca .
  ...
  if (empty($_REQUEST['first']) ) {
    ...
    // display results
    print_results($emp, 'Libro solicitado: ');
...
function get_employees($conn, $firstname)
{
  ...
  $sql = "BEGIN get_book_info(:firstname,  :refcur); END;";
...
function print_results($returned_records, $report_title)
{
  ...
  if (!$returned_records) {
    echo '<p>No libros encontrados</p>';
  }
  else {
    echo '<table border="1">';
